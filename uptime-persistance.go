...
func recordAPIResult(result HttpAPIResult) {
  ...
  if result, exist := cache.cached_results.Endpoints[result.ResultEndpoint]; exist {
    service_document = result
  }

  service_document.LastFetch = result.ResultTimePerformed
  service_document.LastEndpoint = result.ResultEndpoint
  service_document.Fetches = append(service_document.Fetches, result)
